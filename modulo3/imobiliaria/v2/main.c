...
void cadastrarImovel(st_imovel *p, int id, st_prop *proprietario, int qtde_prop)
{
	...
	{	
		...
		if(num_reg != -1)
		{
			...
			{	
				fseek(fptr, 0, 2);
				fread(proprietario,num_reg * sizeof(st_prop), 1, fptr);
				fclose(fptr);
...
void cadastrarLocatario(st_locatario *p, int id, st_imovel *imovel, int qtde_imov, st_prop *proprietario, int qtde_prop)
{
	...
	else
	{
		...
		{	
			...
			{	
				qtde_livres +=1;				
				printf("\nId Imovel: %i\tLogradouro: %s", imovel->id, imovel->end_imovel.logradouro);
			}
		}
		fclose(fptr);
	}
	...
	{	
		...
		num_regImov = buscaImovel(imovel, qtde_imov, num_regImov);		//Verificar se é possível atrelar o Locatario ao Imóvel
		num_regProp = buscaProprietarioPorImovel(proprietario, qtde_prop, num_regImov);		//Faz a busca de um proprietário através do Id/Número de registro de um imóvel				
		op = 'N';
		...
		if(num_regImov != -1 && num_regProp != -1)
		{
			...
			if( (fptr = fopen("imovel.bin", "rb")) == NULL)
				...
				fseek(fptr, num_regImov * sizeof(st_imovel), 0);
				fread(imovel, sizeof(st_imovel), 1, fptr);
				fclose(fptr);
				
				(imovel)->status.locacao.sigla = 'A';				
				(imovel)->status.locacao.reg_loc = p->id;				
...
int buscaImovel(st_imovel *imoveis, int qreg, int num_reg)
{
	...
	else
	{
		...
		{
			//fseek(fptr, 0, 2);
			fread(imoveis, i * sizeof(st_imovel), 1, fptr);
			printf("\nid imovel: %i", imoveis->id);
			if( imoveis->id == num_reg)	
				return i;
		}
		fclose(fptr);
	}
...
int buscaProprietarioPorImovel(st_prop *proprietarios, int qreg, int id_casa)
{
	...
	{	
		...
		{
			for(j=0; j<5; j++)
				if( (proprietarios + i)->casa[j].num_casa == id_casa) 
					return i;
		}
		fclose(fptr);
	}	
...
int buscaCasa(st_prop *proprietarios, int qreg, int i_prop, int id_casa)
{
	...
	else
	{
		...
		for(i=0;i<5;i++)
			if( (proprietarios + i_prop)->casa[i].num_casa == id_casa)
			    return i;
		}	
